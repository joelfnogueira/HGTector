...
try:
    ...
    def load_configs(): return {}
    def get_config(obj, key, cfg_path): pass
    def arg2bool(val):
        if isinstance(val, bool): return val
        return str(val).lower() in ('yes', 'true', '1')
    def list_from_param(param):
        if isinstance(param, list): return param
    ...
    def contain_words(name, words): return any(w in name for w in words)
    def is_latin(name): return True
    def is_capital(name): return name[0].isupper() if name else False
    def taxid_at_rank(tid, rank, taxdump): return tid
    def taxids_at_ranks(tid, ranks, taxdump): return [tid] * len(ranks)
    def is_ancestral(tid, ancestors, taxdump): return tid in ancestors
    def find_lca(tids, taxdump): return tids[0]
    def rank_plural(rank): return rank + 's'
...
arguments = [
    ...
    ['--overwrite',  'overwrite existing files with newly downloaded ones; '
                     'otherwise use existing files whenever available, i.e., '
...
class Database(object):
    ...
    def download_genomes(self):
        ...
        for row in self.df.itertuples():
            ...
            for attempt in range(self.retries):
                try:
                    self._download_file(url, lfile, retries=1)
                    print('  ' + g, flush=True)
                    success = True
                    break
    ...
    def extract_genomes(self):
        ...
        for row in self.df.itertuples():
            ...
            g2n[g], g2aa[g] = 0, 0
            stem = row.ftp_path.rstrip('/').rsplit('/', 1)[-1]
            lfile = join(ldir, f'{stem}_protein.faa.gz')
            if not isfile(lfile):
                print(f'  missing file {stem}_protein.faa.gz, skipping genome {g}', flush=True)
                continue
    ...
    def genome_lineages(self):
        # taxonomic ranks to include (all except species)
        ranks = ['superkingdom', 'kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'species']
        rank_cols = ranks[:-1]  # up to genus
        # apply taxids_at_ranks and expand into separate columns
        taxid_series = self.df['taxid'].apply(lambda x: pd.Series(taxids_at_ranks(x, rank_cols, self.taxdump)))
        # ensure the resulting DataFrame has the correct number of columns
        if taxid_series.shape[1] != len(rank_cols):
            # add missing columns as empty strings
            for i in range(taxid_series.shape[1], len(rank_cols)):
                taxid_series[i] = ''
        taxid_series.columns = rank_cols
        self.df = pd.concat([self.df, taxid_series], axis=1)
        ...
        # merge superkingdom and kingdom: use kingdom if available, otherwise superkingdom
        self.df['kingdom'] = self.df.apply(
            lambda row: row['kingdom'] if pd.notna(row['kingdom']) and row['kingdom'] != '' else row['superkingdom'],
            axis=1
        )

        # generate lineage string (skip superkingdom, start from kingdom)
        lineage_ranks = ranks[1:]  # kingdom, phylum, ..., species
        self.df['lineage'] = self.df[lineage_ranks].fillna('').apply(
            lambda col: col.apply(lambda val, name: '{}__{}'.format(name[0], self.taxdump[val]['name'] if val else ''),
                                  args=(col.name,))
        ).apply('; '.join, axis=1)
